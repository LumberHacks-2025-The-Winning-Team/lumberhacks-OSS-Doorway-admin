...
function findTestFile(quest, task) {
    try {
        // Search for files matching Q#T#-*.py pattern in src/tests directory
        const testsDir = path.join(process.cwd(), 'src', 'tests');
        const pattern = `${quest}${task}-*.(py|json)`;
...
async function runTest(context) {
    try {
        ...
        // Only allow tests for Q3T1
        if (extracted.quest !== "Q3" || extracted.task !== "T1") {
            await issueComment(context, `❌ Tests are only available for Q3 T1. You requested ${extracted.quest} ${extracted.task}.`);
            return;
        }

        // Find test file in src/tests directory
        const testFile = findTestFile(extracted.quest, extracted.task);

        if (!testFile) {
            await issueComment(context, `❌ Test file not found for ${extracted.quest} ${extracted.task}. Expected pattern: ${extracted.quest}${extracted.task}-*.py in src/tests directory`);
            return;
        }
